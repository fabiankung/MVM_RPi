...
camera.init()
mycam = camera.Camera(camera.list_cameras()[0],(_imgwidth_ori,_imgheight_ori),'RGB')
mycam.start()
...
while is_running:
    ...
    imgB = imgnp[:,:,2]                                 # Extract the B component.
    # I = 0.250R + 0.625G + 0.125B = (2R + 5G + B)/8
    #imgI = (2*imgR + 5*imgG + imgB)>>3                  # Compute the luminance or Y value from RGB.
    imgI = 2*imgR + 5*imgG + imgB                       # Basically we are manipulating at the bit level.
    imgI = np.right_shift(imgI,3)                       # Make sure the elements in the array are unsigned integers.
    ...
    screen.blit(img, (0,0))                             # Copy the surface using block transfer to display surface.
